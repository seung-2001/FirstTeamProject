...
//				}
	...
	private void findAll() {
		...
		for (Customer customer : customers) {
			System.out.print("고객 아이디 : " + customer.getCustomerId() + "\t고객명 : " + customer.getName() + "\t이메일 : "
					+ customer.getEmail() + "\t전화번호 : " + customer.getPhone() + "\t등록일 : " + customer.getCreateDate());
			System.out.println("");
		}
	...
	private void findById() {
		...
		if (customers.isEmpty()) {
			...
			for (Customer customer : customers) {
				System.out.print("고객 아이디 : " + customer.getCustomerId() + "\t고객명 : " + customer.getName() + "\t이메일 : "
						+ customer.getEmail() + "\t전화번호 : " + customer.getPhone() + "\t등록일 : "
	...
	private int login() {
		...
		String phone = sc.nextLine();
		int islogined = 0;
		Customer customer = cc.login(email, phone);
		if (customer != null) {
			System.out.println(customer.getName() + "님 로그인 성공!");
			islogined = 1;

		} else {
			System.out.println("로그인 실패: 이메일 또는 전화번호 확인");
			islogined = 0;
		}
		return islogined;
	}
